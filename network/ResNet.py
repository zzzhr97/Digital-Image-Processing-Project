...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(self.in_channel, self.init_channel, kernel_size=7, stride=2,
                            padding=3, bias=False)   # output_size/2
        self.bn1 = nn.BatchNorm2d(self.in_channel)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  # output_size/2

        # 浅层的stride=1，深层的stride=2
        # block：定义的两种残差模块
        # blocks_num：模块中残差块的个数
